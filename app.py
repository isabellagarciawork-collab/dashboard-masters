...
def run():
    ...
    st.markdown("Para una **ingeniera industrial** que busca un máster mixto (gerencial + analítico).")
    ...
    if filtered_df.empty:
        ...
        st.dataframe(filtered_df[display_cols].style.format({
            'Puntaje Final': '{:.1f}',
            'ajuste_perfil_score': '{:.0f}',
            'analitico_score': '{:.0f}',
            'gerencial_score': '{:.0f}',
            'practico_total_score': '{:.0f}',
            'empleabilidad_total_score': '{:.0f}',
            'costo_score': '{:.0f}',
            'precio_total_eur': '€{:,.0f}'
        }))
        ...
        with c1:
            ...
            st.scatter_chart(chart_data, x='precio_total_eur', y='Puntaje Final', size='ECTS', color='#3498db')
        with c2:
            st.markdown("**Comparativa en Radar**")
            options = filtered_df['Programa'].tolist()
            selected_programs = st.multiselect("Selecciona hasta 4 programas para comparar:", options=options, default=options[:min(4, len(options))], max_selections=4)
            if selected_programs:
                ...
                for program in selected_programs:
                    ...
                    values = [
                        program_data['ajuste_perfil_score'], program_data['analitico_score'],
                        program_data['gerencial_score'], program_data['practico_total_score'],
                        program_data['empleabilidad_total_score'], program_data['costo_score']
                    ]
                    radar_fig.add_trace(go.Scatterpolar(r=values, theta=categories, fill='toself', name=program))
        ...
        program_to_detail = st.selectbox("Selecciona un programa para ver el detalle:", options=detail_options)
        if program_to_detail:
            ...
            recommendation_text = f"""Para una ingeniera industrial, el programa **{detail_data['Programa']}** de *{detail_data['universidad']}* tiene un **ajuste profesional {get_ajuste_text(ajuste_score)}** (score {ajuste_score:.0f}).
